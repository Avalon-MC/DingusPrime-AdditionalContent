...
public class GameboyDisplay implements Display, Runnable {
    ...
    public void run() {
        ...
        while (!doStop) {
            ...
            if (doRefresh) {
                GameboyMenuScreen.GBImageBuffer.setRGB(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, rgb, 0, DISPLAY_WIDTH);
                for (int x = 0; x < 160; x++) {
                    for (int y = 0; y < 144; y++)
                    {
                        int rgb = GameboyMenuScreen.GBImageBuffer.getRGB(x,y);

                        //int newRGB = NativeImage.combine(255, (rgb) & 0x000000FF, (rgb >>8 ) & 0x000000FF, (rgb >> 16) & 0x000000FF);
                        //Probably wrong
                        int newRGB = new Color((rgb) & 0x000000FF, (rgb >>8 ) & 0x000000FF, (rgb >> 16) & 0x000000FF, 255).getRGB();

                        GameboyMenuScreen.imageBuffer.getPixels().setPixelRGBA(x,y,newRGB);
                    }
                }
            ...
            GameboyMenuScreen.imageBuffer.upload();
